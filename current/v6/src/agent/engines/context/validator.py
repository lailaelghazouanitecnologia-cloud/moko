...
class PreWriteValidator:
    ...
    def check_imports(self, code: str, from_file: str,
        ...
        for m in _IMPORT_RE.finditer(code):
            ...
            # Resolve import path to a file (safe path resolution)
            from pathlib import PurePosixPath
            from_dir = PurePosixPath(from_file).parent
            resolved = str((from_dir / import_path).as_posix())
            ...
            for part in resolved.split("/"):
                if part == "..":
                    ...
                    parts.append(part)
            resolved = "/".join(parts)
            # Block path traversal: resolved must not escape project
            if resolved.startswith("/") or ".." in resolved:
                result.add_error(
                    "import",
                    f"'{import_path}' resolves outside project boundary",
                    suggestion="use relative imports within the project",
                )
                continue
