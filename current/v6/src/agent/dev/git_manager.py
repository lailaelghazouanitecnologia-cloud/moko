...
# Validation: git branch/ref names must match this pattern
_SAFE_GIT_REF = re.compile(r'^[a-zA-Z0-9][a-zA-Z0-9_./-]*$')
...
def _validate_ref(name: str) -> None:
    ...
    if not _SAFE_GIT_REF.match(name) or '..' in name:
        raise GitError(f"Invalid git ref name: {name!r}")
