...
# Validation: allowed characters for filter values (prevents SQL injection in LanceDB WHERE)
...
class VectorStore:
    ...
    def count(self, project: str = None) -> int:
        ...
        try:
            if project:
                if not _SAFE_IDENTIFIER.match(project):
                    return 0
    ...
    def search(self, query: str, projects: list[str] = None,
        ...
        # Apply structured filters — validate inputs to prevent injection
        where_clauses = []
        if projects:
            safe_projects = [p for p in projects if _SAFE_IDENTIFIER.match(p)]
            if safe_projects:
                proj_list = ", ".join(f"'{p}'" for p in safe_projects)
                where_clauses.append(f"project IN ({proj_list})")
        if descriptor_type and _SAFE_IDENTIFIER.match(descriptor_type):
            where_clauses.append(f"descriptor_type = '{descriptor_type}'")
        if layer and _SAFE_IDENTIFIER.match(layer):
    ...
    def search_by_symbol(self, symbol_name: str,
        ...
        # Validate symbol_name to prevent injection
        if not _SAFE_IDENTIFIER.match(symbol_name):
            return []
        where = (
