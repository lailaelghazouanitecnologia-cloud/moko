...
pub fn emit_dot(wg: &WorkspaceGraph, out_dir: &Path) {
    ...
    for mg in &wg.module_graphs {
        ...
        std::fs::write(out_dir.join(&filename), dot).unwrap();
    }

    // Combined graph with all modules as subgraphs
    let combined = render_combined(wg);
    std::fs::write(out_dir.join("combined.dot"), combined).unwrap();
